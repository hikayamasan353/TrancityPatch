	...
	public class Logger
	{
		...
		public static void Initialize(Assembly app)
		{
			...
			try
			{
				...
				RawLog(string.Format("Build date: {0}, {1}", date.ToShortDateString(), date.ToLongTimeString()));
				date = Utilities.GetBuildDate(thisAssembly);
				RawLog(string.Format("Uses {0} v{1} (build date: {2}, {3})", thisAssembly.GetName().Name, fileVersion.FileVersion, date.ToShortDateString(), date.ToLongTimeString()));
				DebugLog(thisAssembly.GetName().Name, "compiled in this mode");
				RawLog(string.Format("Running on: {0}", Environment.OSVersion.VersionString));
				RawLog("Executable path: " + Application.StartupPath);
				RawLog("Date: " + DateTime.Now.ToShortDateString());
//				Log("Log started");
				_initialized = true;
			}
			catch (Exception exc)
...
//				MessageBox.Show("Cannot start logger. Reason:\n\n" + exc.ToString());
				// считай что сделано:
				ExceptionHandlerForm.ShowException(exc, "Can not start logger!");
			}
		}
