...
namespace Trancity
{
	public abstract class Трамвай : Рельсовый_Транспорт
	{
        ...
        /// <summary>
        /// Обесточен ли трамвай?
        /// </summary>
        public bool обесточен
        {
            get
            {
                //Если токоприемник опущен или обесточен
                if (!токоприёмник.поднят || токоприемник_обесточен)
                    return true; //Да
                else
                {
                    //Если провода не существует
                    if (токоприёмник.Провод == null)
                        return true; //Да, откуда берется электричество??? Закон сохранения энергии никто не отменял!!!
                }
                return !включен; //Если выключен то да
            }
        }
        /// <summary>
        /// Есть ли ток на токоприемнике?
        /// </summary>
        public bool токоприемник_обесточен
        {
            get
            {
                //Если токоприемник не поднят полностью
                if (!токоприёмник.поднят)
                    return true; //Да
                //Если на проводе нету тока или нет самого провода
                if (токоприёмник.Провод.обесточенный || (токоприёмник.Провод == null))
                    return true; //Да
                return false; //В остальных случаях нет
            }
        }
